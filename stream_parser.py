...
#         fallback_embedder parameter from openclaw_hook.py) is now the
#         sole embedder. Removed _check_ollama(), _embed_via_ollama(),
#         and all Ollama state tracking.
#   Why:  StreamParser was written before the ng_embed ecosystem migration
#         (#45, 2026-03-22). Ollama is not the ecosystem standard.
#         ollama_available was always null, chunks_processed always 0.
#   How:  _embed_chunk() calls _fallback_embedder directly. The parameter
#         name is unchanged — openclaw_hook.py passes ng_embed via it.
# -------------------
"""
