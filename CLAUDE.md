...
**StreamParser** (`stream_parser.py`) — Background daemon thread. Consumes text via `feed()`, chunks into overlapping phrases, embeds via Ollama API (`nomic-embed-text` by default, with fallback to ingestor's embedding engine), finds similar nodes in the vector DB, nudges their voltages (`nudge_strength: 0.15`), and triggers hyperedge pattern completion. Creates a continuous attention stream that pre-activates the SNN while text is arriving. **Concurrency (corrected 2026-06-24):** StreamParser's own `threading.Lock` guards ONLY its pause flag — it does NOT share a lock with `graph.step()` (the earlier note claiming so was stale; no shared lock existed, and `openclaw_hook.on_message` does not wrap `step()`+`feed()` together). Its graph-voltage mutations (`_nudge_nodes` + `_trigger_completions`, in `_process_text`) acquire the engine's canonical `graph._step_lock` (the same RLock `graph.step()` and the Commons leg-2 read-only perception hold), so all three voltage writers serialize on the ONE lock. Do NOT add a second/parallel lock — reuse `graph._step_lock`. Do not remove that acquisition (it closes the nudge↔step and nudge↔perception voltage races — punchlist #344). Do not call `graph.step()` from inside the parser.
...
**SurfacingMonitor** (`surfacing.py`) — Bounded priority queue (max 5 items) of concepts whose salience score exceeds `min_confidence` (0.3). Being in `fired_node_ids` already means the node crossed its firing threshold; current voltage is not re-checked. Decays each step (`decay_rate: 0.95`) so stale concepts fade. Formats the queue as a context block for prompt injection — associative "remembering" without explicit search. Uses a negated-score max-heap via Python's `heapq`. The `__lt__` inversion in `_SurfacedItem` is intentional — do not "fix" it.

**CESMonitor** (`ces_monitoring.py`) — Three layers: natural language `health_context()` string for prompt injection, rotating file logger to `~/.neurograph/logs/ces.log`, and HTTP dashboard on **127.0.0.1:8847** with JSON endpoints. **Port is 8847** — some older documentation references 8080, which is wrong (punch list #14). Do not reassign this port. Bound to localhost; do not expose it externally. It is not an inter-module communication channel (Law 1).
